...
func main() {
	...
	var probeIntervalMs int
	var probeTimeoutMs int
	flag.StringVar(&mode, "mode", runnerModeScript, "runner mode")
	...
	flag.IntVar(&probeIntervalMs, "probeIntervalMs", 3000, "frontmost probe sample interval")
	flag.IntVar(&probeTimeoutMs, "probeTimeoutMs", 3000, "frontmost probe timeout")
	flag.Parse()

	serviceMode = normalizeRunnerMode(mode)
	...
	if serviceMode == runnerModeFrontmostProbe {
		if err := runFrontmostProbeService(fridaHost, fridaPort, probeIntervalMs, probeTimeoutMs); err != nil {
			_, _ = io.WriteString(os.Stderr, "frontmost probe failed: "+err.Error()+"\n")
			os.Exit(2)
		}
		return
	}
	if scriptPath == "" || eventsURL == "" || processName == "" {
		_, _ = io.WriteString(os.Stderr, "invalid args\n")
		os.Exit(2)
	}
